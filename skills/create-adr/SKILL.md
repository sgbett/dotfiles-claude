...
Read `.architecture/config.yml` to determine ADR naming format:
```yaml
adr:
  numbering_format: date-based  # or: sequential
```

**If `date-based`** (or not specified): Use today's date as prefix
- Format: `yyyymmdd-kebab-case-title.md`
- Example: `20260116-use-react-for-frontend.md`

**If `sequential`**: Find next ADR number
```bash
ls .architecture/decisions/adrs/ | grep -E "^ADR-[0-9]+" | sed 's/ADR-//' | sed 's/-.*//' | sort -n | tail -1
```
- Format: `ADR-XXX-kebab-case-title.md`
- Example: `ADR-004-use-react-for-frontend.md`
...
### 4. Create Filename
Use format determined in step 2:

**Date-based examples**:
- `20260116-use-react-for-frontend.md`
- `20260116-choose-postgresql-database.md`
...
**Invalid blocked**: "../etc/passwd" → sanitised or rejected
...
### 7. Save ADR
Write to `.architecture/decisions/adrs/` using the appropriate filename format:
- Date-based: `yyyymmdd-title.md`
- Sequential: `ADR-XXX-title.md`

### 8. Report to User
```
Created: [Title]

Location: .architecture/decisions/adrs/[filename].md
Status: [Status]
