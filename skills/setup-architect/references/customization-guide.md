...
See [../assets/member-defaults.yml](../assets/member-defaults.yml) for canonical names.

Each member includes:
- `id`: Unique identifier (lowercase, underscores)
- `name`: Display name (**must use mnemonic initials** - see below)
- `title`: Role/title
...
### Naming Convention: Mnemonic Names

**All member names MUST use initials that match their title** as a memory aid:
- "Systems Architect" → SA → **Simon Ashworth**
- "Ruby Expert" → RE → **Rachel Edwards**
- "Performance Specialist" → PS → **Priya Sharma**

**Cultural diversity**: Aim for 50% British names, 50% international names.

See `assets/member-defaults.yml` for the canonical list of core member names.
...
- id: javascript_expert
  name: "James Evans"              # JE = JavaScript Expert (British)
...
```yaml
- id: python_expert
  name: "Paulo Esteves"            # PE = Python Expert (Portuguese)
  title: "Python Expert"
  specialties:
...
```yaml
- id: ruby_expert
  name: "Rachel Edwards"           # RE = Ruby Expert (British)
  title: "Ruby Expert"
  specialties:
    - "Ruby idioms"
    - "Metaprogramming"
  ...
  domains:
    ...
    - "API services"
    - "Background jobs"
  perspective: "Emphasises Ruby's elegance and Rails conventions for rapid development"
```
...
```yaml
- id: java_expert
  name: "Jun Endo"                 # JE = Java Expert (Japanese)
  title: "Java Expert"
  specialties:
    - "Enterprise Java"
    - "Design patterns"
...
```yaml
- id: go_expert
  name: "Greta Eriksson"           # GE = Go Expert (Swedish)
  title: "Go Expert"
  specialties:
    - "Idiomatic Go"
...
```yaml
- id: rust_expert
  name: "Ravi Eshwar"              # RE = Rust Expert (Indian)
  title: "Rust Expert"
  specialties:
    - "Memory safety"
  ...
  domains:
    ...
    - "Embedded systems"
  perspective: "Emphasises Rust's safety guarantees and performance characteristics"
```
...
These core members should remain for all projects. Use the canonical names from `assets/member-defaults.yml`:

| Role | Canonical Name | Initials | Origin |
|------|----------------|----------|--------|
| Systems Architect | Simon Ashworth | SA | British |
| Domain Expert | Diana Ellis | DE | British |
| Security Specialist | Sarah Shaw | SS | British |
| Maintainability Expert | Michael Eastwood | ME | British |
| Performance Specialist | Priya Sharma | PS | Indian |
| Implementation Strategist | Ingrid Svensson | IS | Swedish |
| AI Engineer | Amara Eze | AE | Nigerian |
| YAGNI Guardian | Yuki Goto | YG | Japanese |
